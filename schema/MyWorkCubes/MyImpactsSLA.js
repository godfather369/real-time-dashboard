...
cube(`MyImpactsSLA`, {
	sql: `SELECT DISTINCT(CONCAT(_id, ugId)), _id, tenantId, status, created, ugId FROM(SELECT * FROM(SELECT * FROM (SELECT _id, tenantId, status, created FROM ${impactAssessmentCollection} where ${impactAssessmentCollection}.archived=0) as impacts LEFT JOIN 
