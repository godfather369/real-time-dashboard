...
cube(`MyAlerts`, {
	sql: `SELECT DISTINCT(CONCAT(_id, ugId)), _id, alertCategory, publishedDate, tenantId, status, ugId, owners FROM(SELECT * FROM(SELECT * FROM (SELECT _id, alertCategory, publishedDate, tenantId, status FROM ${alertsCollection} where ${alertsCollection}.archived=0) as alerts LEFT JOIN 
