-- ═══════════════════════════════════════════════════════════════════════════════
-- Open Alerts or IAs by Groups (SLA) — Instantaneous Counts (Scalable, no hardcoding)
-- Uses rolling 90-day window from current date
-- Equivalent of: queries/aggregates/OpenAlertsOrIAsGroupsSLAInstantaneous.js
-- ═══════════════════════════════════════════════════════════════════════════════
SELECT
  g.groups AS groupId,
...
INNER JOIN `RegHub`.`mv_enriched_alerts_groups` g
  ON g._id = a._id
